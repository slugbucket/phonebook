class Extension < ActiveRecord::Base
  ...
  scope :sub_dept_next, ->(subdept) {select(:id, :extension).joins(", sub_department_extension_range_view").where("sub_department_id = ? AND (extension >= first_extension AND extension <= last_extension) AND id NOT IN (SELECT extension_id FROM phones WHERE extension_id IS NOT NULL)", subdept).order(:extension)}
